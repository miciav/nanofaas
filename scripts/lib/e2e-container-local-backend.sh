...
resolve_function_selection() {
  ...
  scenario_require_single_function || fail "container-local supports exactly one selected function"
  FUNCTION_NAME=$(scenario_first_function_key)
  FUNCTION_SLUG=${FUNCTION_NAME//[^[:alnum:]-]/-}
  FUNCTION_IMAGE=$(scenario_function_image "${FUNCTION_NAME}" || true)
  FUNCTION_RUNTIME_KIND=$(scenario_function_runtime "${FUNCTION_NAME}" || true)
  ...
  [[ -n "${FUNCTION_IMAGE}" ]] || fail "selected function '${FUNCTION_NAME}' does not define an image"
}
...
build_selected_function_image() {
  if [[ -z "${FUNCTION_RUNTIME_KIND}" ]]; then
  ...
  case "${FUNCTION_RUNTIME_KIND}" in
    java)
      info "Building Java demo image ${FUNCTION_IMAGE}"
      (cd "${PROJECT_ROOT}" && ./gradlew ":examples:java:${FUNCTION_FAMILY}:bootBuildImage" "-PfunctionImage=${FUNCTION_IMAGE}" --quiet)
      ;;
    java-lite)
      info "Building Java Lite demo image ${FUNCTION_IMAGE}"
      (cd "${PROJECT_ROOT}" && "${RUNTIME_ADAPTER}" build -t "${FUNCTION_IMAGE}" -f "examples/java/${FUNCTION_FAMILY}-lite/Dockerfile" . >/dev/null)
      ;;
    go)
      info "Building Go demo image ${FUNCTION_IMAGE}"
      (cd "${PROJECT_ROOT}" && "${RUNTIME_ADAPTER}" build -t "${FUNCTION_IMAGE}" -f "examples/go/${FUNCTION_FAMILY}/Dockerfile" . >/dev/null)
      ;;
    python)
      info "Building Python demo image ${FUNCTION_IMAGE}"
      (cd "${PROJECT_ROOT}" && "${RUNTIME_ADAPTER}" build -t "${FUNCTION_IMAGE}" -f "examples/python/${FUNCTION_FAMILY}/Dockerfile" . >/dev/null)
      ;;
    exec)
      info "Building exec demo image ${FUNCTION_IMAGE}"
      (cd "${PROJECT_ROOT}" && "${RUNTIME_ADAPTER}" build -t "${FUNCTION_IMAGE}" -f "examples/bash/${FUNCTION_FAMILY}/Dockerfile" . >/dev/null)
      ;;
    *)
      fail "Unsupported selected function runtime: ${FUNCTION_RUNTIME_KIND}"
      ;;
  esac
}

if [[ "${SKIP_FUNCTION_IMAGE_BUILD}" == "true" ]]; then
  info "Skipping selected function image build; using existing image ${FUNCTION_IMAGE}"
else
  build_selected_function_image
fi
...
if [[ -n "${FUNCTION_PAYLOAD_FILE}" ]]; then
  scenario_write_wrapped_input "${FUNCTION_PAYLOAD_FILE}" "${TMP_DIR}/invoke-request.json"
else
  printf '%s\n' '{"input":{"message":"hello-container-local"}}' > "${TMP_DIR}/invoke-request.json"
fi
curl -fsS \
  -H 'Content-Type: application/json' \
  -d @"${TMP_DIR}/invoke-request.json" \
