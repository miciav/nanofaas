...
Python implementations of VM/K3s E2E scenario workflows (M11).

Replaces:
  - scripts/lib/e2e-k3s-curl-backend.sh  (k3s-curl scenario)
  - scripts/lib/e2e-helm-stack-backend.sh (helm-stack scenario)
...
Invoked via:
    controlplane-tool k3s-e2e run k3s-curl
...
def _selected_functions(resolved: "ResolvedScenario | None") -> list[str]:
...
def _function_runtime(fn_key: str, resolved: "ResolvedScenario | None") -> str:
    if resolved is None:
        return "java"
    for fn in resolved.functions:
...
def _function_family(fn_key: str, resolved: "ResolvedScenario | None") -> str | None:
    if resolved is None:
        return None
    for fn in resolved.functions:
...
class K3sCurlRunner:
    ...
    def _build_jars(self) -> None:
        print(f"[k3s-curl] Building jars (runtime={self.runtime})")
    ...
    def _build_images(self) -> None:
        print("[k3s-curl] Building and pushing core images")
        if self.runtime == "rust":
            self._vm_exec(
                f"cd {self._remote_dir} && "
    ...
    def _deploy_platform(self) -> None:
        print("[k3s-curl] Deploying platform to k3s")
        self._vm_exec(
    ...
    def _run_function_workflow(
        ...
        print(f"[k3s-curl] Running function workflow for '{fn_key}'")
    ...
    def run(self, scenario_file: Path | None = None) -> None:
        ...
        print("[k3s-curl] k3s curl E2E workflow PASSED")
...
class HelmStackRunner:
    ...
    def run(self) -> None:
        ...
        print("[helm-stack] Running experiments/e2e-autoscaling.sh")
        subprocess.run(
            ["bash", str(self.repo_root / "experiments" / "e2e-autoscaling.sh")],
            check=True,
            env=env,
        )
