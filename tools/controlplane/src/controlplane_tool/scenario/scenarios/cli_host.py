...
@dataclass
class CliHostPlan:
    """ScenarioPlan Protocol implementation for cli-host.
    ...
    ``ScenarioPlanner.vm_backed_steps`` (plain host ScenarioPlanSteps), not a
    ...
    def _assemble(
        ...
        tasks for cli-host. When *resolve_host* is False the raw step commands are
        kept (used to derive task_ids/phase_titles without a running VM).
        """
        runner = self.runner
