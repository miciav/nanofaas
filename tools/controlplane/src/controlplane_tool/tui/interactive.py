...
from controlplane_tool.cli_validation.cli_test_catalog import list_cli_test_scenarios
...
_IMPLEMENTATION_CHOICES = [
    _choice(
        "Java",
        "java",
        "Use the Java control-plane implementation, with support for both JVM and GraalVM native building paths.",
    ),
    _choice(
        "Rust",
        "rust",
...
def build_profile_interactive(profile_name: str) -> Profile:
    ...
    tests_enabled = questionary.confirm("Run tests after building?", default=True).ask()
