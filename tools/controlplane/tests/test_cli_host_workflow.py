"""Equivalence oracle: the cli-host Workflow must reproduce the legacy planner steps.

The honest Workflow of Tasks (build_cli_host_plan) must yield the exact same
...
behavior-preserving contract of the C4.1 rewrite.

cli-host is a near-TWIN of cli: its steps come from
...
contains only the scenario step (``cli.host_platform_flow``).
"""
...
def test_workflow_task_ids_match_planner_with_bootstrap() -> None:
    ...
    workflow_ids = build_cli_host_plan(runner, request).workflow_task_ids

    assert "vm.ensure_running" in legacy_ids
    assert "vm.ensure_running" in workflow_ids
    assert "cli.host_platform_flow" in workflow_ids
...
def test_workflow_task_ids_match_planner_without_bootstrap() -> None:
    ...
    workflow_ids = build_cli_host_plan(
        runner, request, include_bootstrap=False
    ).workflow_task_ids

    assert legacy_ids == ["cli.host_platform_flow"]
...
def test_phase_titles_match_planner_summaries_with_bootstrap() -> None:
    ...
    plan = build_cli_host_plan(runner, request, include_bootstrap=True)
...
def test_phase_titles_match_planner_summaries_without_bootstrap() -> None:
    ...
    plan = build_cli_host_plan(runner, request, include_bootstrap=False)
    assert plan.phase_titles == legacy_summaries
...
def test_workflow_commands_match_resolved_planner_commands() -> None:
    ...
    cli-host steps are all host steps; the legacy planner keeps <multipass-ip:NAME>
    ...
    plan = build_cli_host_plan(runner, request)
