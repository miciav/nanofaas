...
def test_query_metric_names_returns_set(monkeypatch) -> None:
    ...
    with patch("workflow_tasks.loadtest.prometheus.httpx.get", return_value=_mock_httpx_get(payload)):
...
def test_query_metric_names_raises_on_connection_error(monkeypatch) -> None:
    with patch("workflow_tasks.loadtest.prometheus.httpx.get", side_effect=httpx.RequestError("refused")):
