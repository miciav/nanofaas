...
async fn runtime_config_get_returns_rate_and_revision_when_enabled() {
    ...
    assert_eq!(body["syncQueueEnabled"], false);
    assert_eq!(body["syncQueueAdmissionEnabled"], true);
    assert_eq!(body["syncQueueMaxEstimatedWait"], "PT2S");
    assert_eq!(body["syncQueueMaxQueueWait"], "PT2S");
    assert_eq!(body["syncQueueRetryAfterSeconds"], 2);
}
...
#[tokio::test]
async fn runtime_config_patch_updates_sync_queue_fields() {
    let app = control_plane_rust::app::build_app_with_runtime_config_admin(true);
            ...
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        ...
                        "syncQueueAdmissionEnabled": false,
                        "syncQueueMaxEstimatedWait": "PT1S",
                        "syncQueueMaxQueueWait": "PT5S",
                        "syncQueueRetryAfterSeconds": 7
                    })
                    .to_string(),
                ))
                .unwrap(),
    ...
    assert_eq!(patch_response.status(), StatusCode::OK);
    let patched = response_json(patch_response).await;
    assert_eq!(patched["effectiveConfig"]["syncQueueEnabled"], true);
    assert_eq!(
        patched["effectiveConfig"]["syncQueueAdmissionEnabled"],
        false
    );
    assert_eq!(
        patched["effectiveConfig"]["syncQueueMaxEstimatedWait"],
        "PT1S"
    );
    assert_eq!(patched["effectiveConfig"]["syncQueueMaxQueueWait"], "PT5S");
    assert_eq!(patched["effectiveConfig"]["syncQueueRetryAfterSeconds"], 7);
}

#[tokio::test]
async fn runtime_config_validate_rejects_invalid_effective_sync_queue_settings() {
    let app = control_plane_rust::app::build_app_with_runtime_config_admin(true);
    ...
    let response = app
        ...
        .oneshot(
            Request::builder()
                ...
                .body(Body::from(
                    json!({
                        "syncQueueMaxEstimatedWait": "PT5S"
                    })
                    .to_string(),
                ))
                .unwrap(),
    ...
    assert!(body["errors"].as_array().unwrap().iter().any(|item| item
        .as_str()
        .unwrap_or_default()
        .contains("syncQueueMaxEstimatedWait")));
}

#[tokio::test]
async fn runtime_config_patch_returns_bad_request_for_malformed_duration() {
    let app = control_plane_rust::app::build_app_with_runtime_config_admin(true);
