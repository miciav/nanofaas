...
const IMAGE_TAG: &str = "nanofaas/control-plane-rust-m3:e2e";
...
fn start_environment() -> Result<(DockerContext, String), String> {
    ...
    let network_name = format!("cp-rust-e2e-net-{suffix}");
    docker_cmd(&["network", "create", &network_name], None)?;

    let mut ctx = DockerContext::new(network_name.clone());

    let runtime_name = format!("cp-rust-e2e-runtime-{suffix}");
    docker_cmd(
        &[
            "run",
            "-d",
    ...
    let cp_name = format!("cp-rust-e2e-cp-{suffix}");
    docker_cmd(
        &[
            "run",
            "-d",
...
async fn register_pool_function(client: &Client, base_url: &str, name: &str) -> Result<(), String> {
    ...
    if resp.status().as_u16() != 201 {
        ...
        return Err(format!("register failed with status {}: {}", 201, body));
    }
    Ok(())
}

#[tokio::test]
async fn dockerized_e2eRegisterInvokeAndPoll() {
    let _guard = docker_test_lock().lock().expect("docker e2e lock");
    if !docker_available() {
        eprintln!("skipping dockerized e2e: docker not available");
        return;
    }
    ...
    register_pool_function(&client, &base_url, "e2e-echo")
        .await
        .expect("register pool function");

    let invoke = client
        .post(format!("{base_url}/v1/functions/e2e-echo:invoke"))
        .json(&json!({"input": {"message": "hi"}}))
        .send()
        .await
    ...
    let enqueue = client
        .post(format!("{base_url}/v1/functions/e2e-echo:enqueue"))
    ...
    let enqueue2 = client
        ...
        .send()
        .await
        .expect("enqueue2 request");
    assert_eq!(202, enqueue2.status().as_u16());
    let enqueue2_json: Value = enqueue2.json().await.expect("enqueue2 json");
    assert_eq!(
        execution_id,
        enqueue2_json["executionId"]
            .as_str()
            .expect("execution id 2")
    );

    let drain = client
        .post(format!(
            "{base_url}/v1/internal/functions/e2e-echo:drain-once"
        ))
        .send()
        .await
    ...
    panic!("execution did not reach SUCCESS");
}

#[tokio::test]
async fn dockerized_e2ePrometheusMetricsExposed() {
    let _guard = docker_test_lock().lock().expect("docker e2e lock");
    if !docker_available() {
        eprintln!("skipping dockerized e2e: docker not available");
        return;
    ...
    register_pool_function(&client, &base_url, "metrics-e2e")
        .await
        .expect("register pool function");

    let enqueue = client
        .post(format!("{base_url}/v1/functions/metrics-e2e:enqueue"))
        .json(&json!({"input": "payload"}))
        .send()
        .await
        .expect("enqueue request");
